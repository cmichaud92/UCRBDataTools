...
field_xlsx_wkbk <- function(.site_data,
    ...
    if (!is.null(.rare_data_2)) {
      ...
      openxlsx::writeData(wb = B, sheet = 2, x = .rare_data_2)                            # Writes data to the blank sheet
      # Add column formatting
    ...
    openxlsx::writeData(wb = B, sheet = 3, x = .site_data)
    ...
    if (!is.null(.ntf_data)){
      ...
      openxlsx::writeData(wb = B, sheet = 4, x = .ntf_data)

      openxlsx::addStyle(B, 4, style = n0,                                             # Associates it with col 16
