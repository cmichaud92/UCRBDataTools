#' Check STReaMS submissions for missing values in non-required fields
#'
...
#'
#' @examples
STReaMS_other_na <- function(.data, .table) {

  if (nrow(.data > 0)) {

    if (grepl("rare", .table, ignore.case = TRUE)) {
      rqd_vec <- tmplt_streams_field$`imp_Rare Fish datasheet`

    } else if (grepl("site", .table, ignore.case = TRUE)) {
      rqd_vec <- tmplt_streams_field$`imp_Sample - Site-effort datasheet`

    } else if (grepl("ntf", .table, ignore.case = TRUE)) {
      rqd_vec <- tmplt_streams_field$`imp_Non-tagged Fish datasheet`
    }
